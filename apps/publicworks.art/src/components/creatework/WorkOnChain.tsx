...
export const WorkOnChain = ({ minter, slug, work }: Props) => {
  ...
  if (!minter) {
    return <>Your work is not instantiated on chain yet. Go back and finish deploying your smart contract!</>;
  }
      ...
      <div>
            ...
            >
              <Form.Group controlId="formAirdropAddresses">
                ...
                <Form.Control
                  ...
                  isInvalid={formik.touched.addresses && !!formik.errors.addresses}
                />
                <Form.Control.Feedback type="invalid">{formik.errors.addresses}</Form.Control.Feedback>
        ...
        <div>
          ...
          <Collapse key={'collapseChangePrice'} in={changePriceVisible}>
            <div>
              {(!!work && !!getMinterQuery.data && <ChangePrice work={work} minter={getMinterQuery.data} />) || null}
            </div>
          </Collapse>
        </div>
      </div>

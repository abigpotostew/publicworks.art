...
### What is a work?
Publicworks is a mint platform for creating generative collections that mint on-demand. A work is a generative collection with NFTs that are created upon mint. NFTs are not pre-generated.

This works by creating code (javascript) inside of a website that generates the image or animation (or other media) based on the hash of the mint transaction.
...
Your project's code is bundled into a zip file and uploaded as a standalone website to IPFS. This code url is stored on chain indefinitely so your work is decentralized. 

For a short period after minting, the public works backend intercepts the mint event, renders a preview image for the nft, and stores that on chain. After that, your work is permanently stored in decentralized hosting.
...
Attributes, also known as NFT traits, can be generated by your code. Public works will extract these from your code and store them into your NFT metadata. 

Generally speaking, attributes should be "human readable". meaning your code should output easy to grok attribute names and values. For example, consider an attribute \`density\` which is a long decimal number \`0.0120987109812\`. And attribute of \`density: 0.0120987109812\` is hard to read. Instead, consider the range. This could be \`Density: Low\` if less than 0.2, \`Density: Medium\` is less than 0.8 and \`Density: High\` otherwise.
...
Your work will run in different environments.
On the publicworks.art preview pipeline: \`isPWPreview()\` will return \`true\`. In this case, your work must render the preview image and then call \`setPreviewReady()\`.
Displaying on publicworks.art \`isPW()\` will return true. But \`isPWPreview()\` will return \`false\`. Animations are allowed in this case.
...
export function img(props: any) {
  ...
  return (
    // <Figure>
    <Container fluid className={"Margin-B-8 Margin-T-8"}>
      <Row>
...
export function Markdown({ markdown }: { markdown: string }) {
  ...
  function h3(props: any) {
    ...
    return (
      <h3 id={slug} className={"Margin-B-8 Margin-T-8"}>
        <a
  ...
  function h4(props: any) {
    ...
    const slug = generateSlug(heading);
    return (
      <h4 id={slug} className={"Margin-B-4 Margin-T-4"}>
        <a

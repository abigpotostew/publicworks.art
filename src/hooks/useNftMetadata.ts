...
export const useNftMetadata = ({ sg721, tokenId }: { sg721: string; tokenId: string }) => {

  const [loading, setLoading] = useState<boolean>(true)
  const [errorFetch, setErrorFetch] = useState<any | null>(null)
  const [metadata, setMetadata] = useState<NftMetadata | null>(null)

  const msgBase64 = useMemo(() => Buffer.from(JSON.stringify({ nft_info: { token_id: tokenId } })).toString('base64'), [tokenId])
